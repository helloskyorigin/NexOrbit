...
export const FollowUpSuggestions: React.FC<FollowUpSuggestionsProps> = ({
  ...
  const suggestions = [
    ...
    { label: 'Prepare a response', prompt: 'Draft a follow-up response email to Rahul regarding the deadline.' },
    { label: 'Summarize this', prompt: 'Give me a 2-sentence summary of these insights.' },
  ];

  return (
    <div className={cn('pt-2 space-y-1.5', className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {suggestions.map((item, i) => (
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-indigo-50 hover:text-indigo-900 border border-slate-200/80 hover:border-indigo-200 text-xs font-medium text-slate-700 transition-all flex items-center gap-1.5 group"
          >
            <Sparkles className="h-3 w-3 text-indigo-500 fill-indigo-500 shrink-0" />
